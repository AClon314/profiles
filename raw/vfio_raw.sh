alias hows-my-gpu='echo "NVIDIA Dedicated Graphics" |\
grep "NVIDIA" &&\
lspci -nnk |\
grep "NVIDIA Corporation GA107M" -A 2 |\
grep "Kernel driver in use" &&\
echo "Intel Integrated Graphics" |\
grep "Intel" &&\
lspci -nnk |\
grep "Intel.*Integrated Graphics Controller" -A 3 |\
grep "Kernel driver in use" &&\
...
alias nvidia-enable='sudo virsh nodedev-reattach pci_0000_01_00_0 &&\
...
alias nvidia-disable='sudo rmmod nvidia_modeset nvidia_uvm nvidia &&\
...
alias active-gpu='glxinfo |\
grep vendor'

alias active-gpu-prime='__NV_PRIME_RENDER_OFFLOAD=1 __GLX_VENDOR_LIBRARY_NAME=nvidia glxinfo |\
grep vendor'


alias unigine-valley='pushd . &&\
cd ~/Downloads/Unigine_Valley-1.0/ &&\
